...
```text
ct-qa-flow             검증 범위, 위험별 시나리오, 실행 결과와 재현 방법
      ↓
ct-code-tree-test      자동화가 필요한 흐름의 테스트                       ← 반복 검증이 필요할 때
      ↓
ct-code-spring         발견한 결함 수정                                    ← 수정 대상이 확정된 뒤
      ↓
ct-qa-flow             수정한 흐름과 영향 범위 재검증                       ← 결함을 수정했을 때
```
...
전 범위를 같은 깊이로 보지 않아요.  
위험이 낮은 흐름은 확인 대상에서 제외한 이유를 남겨요.

### ct-code-tree-test

같은 검증을 다음 배포에서도 반복해야 하면 테스트로 남겨요.  
일회성 확인은 테스트로 만들지 않아요.

### ct-code-spring

결함 수정은 원인이 확정된 뒤에 진행해요.  
증상만 가리는 수정은 재발 조건을 함께 기록해요.

수정 뒤에는 최초 검증 범위와 영향받은 흐름을 `ct-qa-flow`로 다시 확인해요.
