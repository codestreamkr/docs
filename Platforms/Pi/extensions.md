...
외부 도구 연결은 CLI 도구를 설명하는 Skill로 만들거나  
Extension으로 MCP를 붙여요.
...
같은 `SKILL.md`를 Claude Code, Codex, Grok Build과 공유할 수 있어요.  
현재 사용자 Skill과 호출 예제는 [Skill 안내](./skills.md)에서 확인해요.

## Prompt Template

자주 쓰는 요청 문구를 명령처럼 사용해요.

- 위치: `~/.pi/agent/prompts/`, `<repo>/.pi/prompts/`
- 호출: `/템플릿이름`

판단 기준과 절차가 중요한 작업은  
Prompt Template이 아니라 Skill로 만들어요.
...
Extension, Skill, Prompt Template과 Theme를  
하나의 배포 단위로 묶어요.
...
- npm 또는 git으로 배포해요.
- `package.json`의 `pi` 항목으로 포함 자원을 지정하고,  
  없으면 관례 디렉터리에서 찾아요.
- Package는 전체 시스템 권한으로 실행되므로  
  설치 전에 코드를 확인해요.

## SDK와 RPC

에이전트를 다른 프로그램에 내장할 때 사용해요.

- SDK: 자동화 스크립트나 서비스에 Pi를 직접 포함해요.
- RPC: `--mode rpc`로 다른 언어에서 Pi를 제어해요.
- JSON 출력: `--mode json`으로 실행 결과를 수집해요.
