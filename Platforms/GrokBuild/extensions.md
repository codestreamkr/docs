...
`SKILL.md`의 필수 항목은 `name`과 `description`이에요.  
추가 경로는 Config의 `[skills] paths`로 등록해요.
...
- Role로 기본 권한과 모델을,  
  Persona로 응답 방식을 지정해요.
- 세션 전체에서 끄려면 `--no-subagents`를 사용해요.

## Custom Model

xAI 외 모델이나 사내 게이트웨이를  
모델 목록에 추가해요.
...
- 인증값은 `api_key`, `env_key` 또는  
  회전 토큰용 `auth_provider`로 지정해요.
...
- 사용자 범위는 `~/.grok/config.toml`,  
  저장소 범위는 `<repo>/.grok/config.toml`에 둬요.
...
- 실행 조건: 프로젝트 Hook은 `/hooks-trust`로 폴더를 신뢰해야 해요
- 확인: `/hooks`

프로젝트의 의미와 맥락을 판단해야 하는 기준은  
Hook이 아니라 `AGENTS.md`나 Skill에서 제공해요.
...
- 관리: `/plugins`
- 마켓플레이스 소스는 Config의 `[[marketplace.sources]]`에 둬요
- 프로젝트 Plugin은 별도로 신뢰해야 실행돼요.  
  `~/.grok/plugins/`는 자동으로 신뢰해요.
