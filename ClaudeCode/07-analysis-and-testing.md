...
1. [사전작업](#사전작업)
2. [커맨드 소개](#커맨드-소개)
3. [ct:calltree 활용](#ctcalltree-활용)
4. [ct:calltreeTest 활용](#ctcalltreetest-활용)
5. [ct:tran-plan 활용](#cttran-plan-활용)
6. [이어서 쓰는 흐름](#이어서-쓰는-흐름)
7. [알아두면 좋은 것](#알아두면-좋은-것)

---

## 사전작업

이 문서의 `ct:*` 호출은 Claude Code 기본 제공 명령이 아니라 커스텀 커맨드 기준이다.

### 준비가 필요한 기능

| 기능 | 구분 | 준비 기준 |
|------|------|-----------|
| `/ct:calltree` | Custom command | 호출 관계 분석 커맨드가 준비되어 있어야 한다 |
| `/ct:calltreeTest` | Custom command | calltree 기반 테스트 생성 커맨드가 준비되어 있어야 한다 |
| `/ct:tran-plan` | Custom command | 전환 계획 작성 커맨드가 준비되어 있어야 한다 |

이후 예시는 위 커맨드가 준비된 상태에서 실행한다.

---

## 커맨드 소개

세 가지 커스텀 커맨드가 있다. 역할이 다르기 때문에 상황에 따라 하나만 쓰거나 순서에 맞게 이어서 쓴다.
...
- 2026-04-30: 목차 추가, `ct:tran-plan` 예제와 커맨드 연결 흐름 정리
- 2026-04-10: 최초 생성
