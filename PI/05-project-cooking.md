...
다른 접근을 실험하려면 `/tree`, `/fork`, `/clone`을 사용한다.

| 명령어 | 쓰는 상황 |
|---|---|
| `/tree` | 같은 세션 안에서 이전 지점으로 이동해 다른 방향 시도 |
| `/fork` | 과거 사용자 메시지에서 새 세션 파일 생성 |
| `/clone` | 현재 활성 흐름을 새 세션 파일로 복제 |
...
- 2026-05-11: 반복 작업을 Extension, custom tool, Pi package로 옮기는 흐름 추가
- 2026-05-11: 최초 생성
