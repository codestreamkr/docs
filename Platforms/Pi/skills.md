...
```text
/skill:ct-plan-work 주문 취소 기능의 사용자 흐름과 성공 기준을 정리해줘
/skill:ct-plan-work .docs/payment-design.md의 근거와 위험을 검토해줘
/skill:ct-plan-work 회원 모듈의 구조 전환 계획을 작성해줘
```
...
```text
/skill:ct-code-spring 주문 취소 트랜잭션을 구현해줘
/skill:ct-code-spring Security 설정을 읽기 전용으로 검토해줘
/skill:ct-code-spring 기존 방식에 맞는 API 예외 응답 컴포넌트를 추가해줘
```
...
```text
/skill:ct-run-script macOS 개발 실행 스크립트를 만들어줘
/skill:ct-run-script Windows에서 API와 프론트를 함께 실행하게 해줘
```
...
```text
/skill:ct-code-tree OrderController.cancel의 호출 흐름을 분석해줘
```
...
```text
/skill:ct-code-tree-test .docs/callTree-OrderController-cancel.md를 근거로 테스트를 작성해줘
```
...
```text
/skill:ct-qa-flow 주문 취소 기능을 오픈 전 검증해줘
/skill:ct-qa-flow 로그인 API와 화면 흐름의 회귀 범위를 정리해줘
```
...
```text
/skill:ct-data-query 주문 조회 쿼리가 느린 원인을 분석해줘
/skill:ct-data-query src/main/resources/mapper/OrderMapper.xml의 findOrders
```
...
```text
/skill:ct-plan-ext 기존 PG 연동을 새 공급자로 이관하는 계획을 작성해줘
/skill:ct-plan-ext 결제 승인·취소 연동 구조를 검토해줘
```
...
```text
/skill:ct-docs-impl OrderService.cancel의 현재 구현 문서를 만들어줘
/skill:ct-docs-impl .docs/order-cancel.md를 현재 코드에 맞춰줘
/skill:ct-docs-impl .docs/order-*.md를 하나의 구현 문서로 합쳐줘
```
...
```text
/skill:ct-wiki-api 필요한 환경변수가 설정됐는지 확인해줘
/skill:ct-wiki-api 결제 개편 문서를 검색해줘
/skill:ct-wiki-api 333 페이지와 댓글을 조회해줘
/skill:ct-wiki-api 333 페이지의 승인 정책 문단을 수정해줘
```
...
```text
/skill:ct-wiki-ops 결제 개편의 결정 사항을 위키 근거로 알려줘
/skill:ct-wiki-ops .wiki/payment.md의 근거와 현재성을 검증해줘
