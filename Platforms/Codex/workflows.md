...
여러 결과가 필요한 작업에서는  
앞 단계의 확정된 결과를 다음 Skill의 입력으로 연결해요.

문제 유형별 흐름과 단계별 판단 기준은 [Playbook](../../Playbooks/README.md)에서 관리해요.  
이 문서는 Codex에서의 호출과 연결 방법만 다뤄요.

## 호출과 연결

Skill 이름 앞에 `$`를 붙이고,  
앞 단계가 만든 결과를 다음 호출에서 지정해요.
...
각 Skill의 입력과 결과는 [사용자 Skill](./skills.md)에서,  
세션 명령은 [명령 확인](./commands.md)에서 확인해요.
