...
xAI 외 모델이나 사내 게이트웨이를 모델 목록에 추가해요.
...
프로젝트의 의미와 맥락을 판단해야 하는 기준은 Hook이 아니라 `AGENTS.md`나 Skill에서 제공해요.

## Plugin

도구, Skill과 MCP 서버를 하나의 설치 단위로 배포할 때 사용해요.

- 위치: `<repo>/.grok/plugins/`, `~/.grok/plugins/`
- 관리: `/plugins`
- 마켓플레이스 소스는 Config의 `[[marketplace.sources]]`에 둬요
- 프로젝트 Plugin은 별도로 신뢰해야 실행돼요. `~/.grok/plugins/`는 자동으로 신뢰해요.
