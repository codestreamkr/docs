...
필요한 테스트 케이스를 설계해 실행한 뒤 결과를 보고해줘.
Git 변경 대상이 아닌 파일은 검증하지 말고 건너뛰어줘.
에이전트가 할 수 있는 검증은 먼저 실행하고, 사람 조작이 필요한 항목만 요청해줘.
로그인이나 화면 조작이 필요하면 브레이크포인트를 먼저 준비한 뒤,
내가 수행할 동작을 하나만 안내해줘.

JVM은 로그포인트로 확인하고, 값이 필요할 때만 중단해줘.
JS는 수동 PAUSE 말고 확인할 줄이나 기존 주기 타이머에서 멈춰줘.
JavaScript Debug는 IntelliJ가 연 Chromium에서 하고, 로그인이 필요하면 그 창 상태를 확인해줘.

실제 결제나 외부 상태 변경은 하지 말고, 결제 SDK 호출 직전까지만 검증해줘.
테스트 코드는 필요하면 추가해도 되지만 운영 코드는 수정하지 마.

각 결과는 PASS / FAIL / 미검증 / 건너뜀으로 구분하고, 실제 관찰값과 근거를 남겨줘.
범위와 방식별로 결과를 남기고, 한 요청에 여러 값을 넣었으면 항목별 결과와 전체 결과를 함께 남겨줘.
```

대상 파일·심볼·화면을 제한할 때만 아래에 경로를 추가해요.  
없으면 Git 작업 트리의 변경 파일만 대상으로 해요.
...
- 요청에 적힌 범위와 방식만 실행해요.
- Git 변경 파일이 아니면 건너요. 건너뛴 항목은 PASS가 아니에요.
...
- JS는 수동 `PAUSE`를 쓰지 않아요. 확인할 줄이나 페이지에 이미 있는 주기 타이머에서 멈춘 뒤 필요한 클릭만 예약해요.
