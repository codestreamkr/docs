...
ct-wiki-ops            원문 보관
      ↓
ct-wiki-ops            위키 구조에 반영
      ↓
ct-wiki-ops            근거, 현재성과 링크 점검
```

원격 위키를 쓰지 않으면 원문 보관부터 시작해요.

## 단계별로 확정할 것

### ct-wiki-api

원격 Confluence 호환 API 작업을 담당해요.

- 대상 페이지와 하위 페이지 범위
- 조회인지 변경인지
- 변경이면 먼저 dry-run으로 결과를 확인해요

### ct-wiki-ops — 원문 보관

원문을 가공하지 않고 그대로 보관해요.  
요약과 판단은 다음 단계의 일이에요.
...
중복이 쌓이면 정리 후보를 확인하고, 기간별 정리가 필요하면 변경과 남은 작업을 요약해요.
