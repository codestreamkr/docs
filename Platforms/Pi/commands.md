...
| --- | --- |
| `/model` | 사용할 모델 선택 |
| `/settings` | thinking 수준,<br>테마와 전송 방식 변경 |
| `/reload` | Extension, Skill, Prompt와<br>지침 파일 다시 읽기 |
| `/hotkeys` | 단축키 전체 확인 |
...
| 명령 | 용도 |
| --- | --- |
| `/session` | 세션 파일, ID,<br>메시지 수와 비용 확인 |
...
사용자 Skill은 `/skill:ct-*`로,  
Prompt Template은 `/템플릿이름`으로 호출해요.
