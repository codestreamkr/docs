...
이 실습에서는 이 문서 저장소에 포함된 예시 package를 설치한다.

### 목표

- Extension, skill, prompt template을 직접 만들지 않고 설치한다.
- 프로젝트 로컬 설치와 임시 실행을 구분한다.
...
예시 package를 설치했다면 `/ready-pr`을 사용할 수 있다.
...
예시 package를 설치했다면 `project-check` skill을 사용할 수 있다.
...
예시 package의 `package_scripts` 도구는 `package.json`의 scripts를 조회한다.
...
- 모델이 `package_scripts` 도구를 호출할 수 있다.
...
- 2026-05-11: Jira, Confluence custom tool 활용 예시 추가
- 2026-05-11: 반복 Extension 예제를 줄이고 다양한 하네스 활용 패턴으로 재구성
- 2026-05-11: Pi package 설치 실습 추가
- 2026-05-11: 02의 Pi 강점 항목과 이어지도록 SDK/RPC, custom provider 실습 추가
- 2026-05-11: Pi 하네스 활용 실습 중심으로 재작성
