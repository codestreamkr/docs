...
Claude Code 실행 설정을 역할에 맞는 위치에 둬요.
...
| `<repo>/CLAUDE.md` 또는<br>`<repo>/.claude/CLAUDE.md` | 프로젝트 작업 기준과<br>정본 문서 안내 |
...
Claude Code가 제공하는 Bundled Skill은 제품이 관리하며  
사용자 Skill과 이름이 겹치면 사용자 Skill이 우선해요.
...
- 특정 경로에만 필요한 기준은 `.claude/rules/`에  
  `paths` 조건과 함께 둬요.
- 다른 파일을 함께 읽히려면 `@path/to/file` 형식으로 가져와요.
- `/init`으로 초안을 만들고  
  프로젝트에서 직접 확인한 내용을 보완해요.

Claude Code는 `AGENTS.md`를 직접 읽지 않아요.  
`AGENTS.md`를 쓰는 저장소는 `CLAUDE.md`에서 `@AGENTS.md`로 가져와  
같은 기준을 공유해요.
...
우선순위는 관리 정책, CLI 옵션, `settings.local.json`,  
`settings.json`, 사용자 설정 순이에요.  
프로젝트의 코드 규칙과 문서 책임은 Settings가 아니라  
`CLAUDE.md`에서 관리해요.  
비밀값은 설정 본문에 적지 않고 환경 변수를 사용해요.
