...
기존 동작을 유지하면서  
현재 흐름, 목표 책임과 단계별 적용 순서를 함께 정해요.
...
기존 구조를 유지한 채 테스트만 필요하면  
`analyze` 다음에 바로 `test`를 사용해요.
...
- 바뀌는 대상이 외부 공급자면  
  [06](./06-integrate-external.md)에서 계약부터 설계해요.
- 전환 범위를 아직 못 정했으면  
  [07](./07-improve-module.md)에서 우선순위를 먼저 정해요.
- 단계별 회귀 검증이 커지면  
  [08](./08-verify-before-release.md)을 함께 사용해요.
