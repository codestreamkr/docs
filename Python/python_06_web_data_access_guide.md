...
SQLAlchemy 2.x는 타입 힌트 기반 매핑을 사용해요.
...
목록을 순회하면서 연관 객체에 접근하면 지연 로딩이 반복 실행돼요.
...
실행 모델 판단 기준은 [실행과 운영](./python_08_web_runtime_operations_guide.md)에서 다뤄요.

## 마이그레이션

스키마 변경은 항상 파일로 남기고 코드와 함께 리뷰해요.

Alembic 사용 절차는 다음과 같아요.
...
애플리케이션과 스키마가 동시에 바뀌면 배포 중 오류가 발생해요.  
컬럼 변경은 여러 단계로 나눠요.

1. 새 컬럼을 nullable로 추가해요.
2. 새 컬럼에도 값을 쓰는 코드를 배포해요.
3. 기존 데이터를 새 컬럼으로 이전해요.
4. 새 컬럼만 읽는 코드를 배포해요.
5. 제약을 추가하고 기존 컬럼을 제거해요.

## 다음 단계

- 요청·응답 계약과 인증 기준을 정해요: [API 계약과 인증](./python_07_web_api_contract_guide.md)
