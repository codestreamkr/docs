...
에이전트가 IntelliJ 세션을 다룰 때 항상 지켜요.  
하위 문서의 화이트리스트는 이 원칙에 더해 적용돼요.

- **세션**: 이전 대화와 디버거 상태를 가정하지 않아요. 지금 열린 실행 구성과 로그인된 화면만 사용해요.
- **변경 금지**: 운영 코드, 실행 구성, 설정 파일
- **사용자 상태**: 사용자 브레이크포인트를 바꾸지 않아요. 실행 중인 세션을 임의로 종료하지 않아요.
- **보고**: `PASS` / `FAIL` / `미검증` / `건너뜀`과 관찰값, 근거를 남겨요. 확인하지 못했거나 건너뛴 항목은 PASS가 아니에요.
