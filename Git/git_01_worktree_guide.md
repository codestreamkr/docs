...
Git 작업은 현재 상태를 정확히 읽는 것부터 시작해요.
...
- `git diff`: 빠진 변경과 의도하지 않은 변경을 찾아요.
- `git diff --staged`: 실제 커밋 범위를 최종 확인해요.
- `git log`: 현재 브랜치와 기준 브랜치의 위치를 확인해요.
