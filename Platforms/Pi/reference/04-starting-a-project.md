...
Windows에서는 Git Bash 또는 다른 bash가 필요해요.

## 1. 프로젝트 루트 확인

먼저 작업할 프로젝트 루트를 확정해요.
...
- `pwd`와 Git 루트가 다르면 의도한 Pi 실행 위치인지 확인해요.
- 모노레포는 작업 대상 모듈과 Pi 실행 위치를 함께 정해요.
- 이후 명령은 확정한 프로젝트 루트에서 실행해요.

## 2. 작업 전 Git 상태 확인

기존 변경 사항을 작업 기준으로 기록해요.
...
Pi 설정을 추가하기 전에 프로젝트의 현재 검증 기준을 확인해요.
...
- 기준: 변경 전 실행 결과와 실패 항목을 남겨요.
- 결과: 이후 Pi 작업으로 새로 발생한 실패와 기존 실패를 구분해요.

## 4. Project Trust 결정 후 Pi 실행

프로젝트 로컬 리소스의 소스와 동작 범위를 확인한 뒤 Pi를 실행해요.
...
`/trust`는 `~/.pi/agent/trust.json`을 갱신해요.  
현재 세션은 다시 로드되지 않으므로 Pi를 재시작해요.

비대화형 실행은 확인 창을 표시하지 않아요.  
한 번의 실행에만 신뢰 결정을 적용해요.
...
프로젝트별 설정은 `.pi/settings.json`에 둬요.
...
- 프로젝트에만 적용할 값은 `.pi/settings.json`에 둬요.
- 개인 취향은 `~/.pi/agent/settings.json`에 둬요.
- 팀과 공유할 package는 프로젝트 설정에 둬요.
- 민감한 키나 개인 인증 정보는 프로젝트 설정에 넣지 않아요.

처음으로 신뢰 대상 리소스를 만들었다면 `/trust`로 현재 프로젝트의 결정을 저장하고 Pi를 재시작해요.

## 8. 프로젝트 Extension 위치 만들기

프로젝트 전용 하네스 조정은 `.pi/extensions`에 둬요.

```bash
mkdir -p .pi/extensions
```

처음에는 작은 Extension부터 둬요.
...
Extension을 추가한 뒤에는 Pi에서 다시 로드해요.
...
이미 만들어진 하네스 리소스가 있으면 package로 설치해요.

프로젝트에만 설치하려면 `-l`을 사용해요.
...
로컬 예제 구성은 [basic-pi-package](../examples/basic-pi-package/)를 참고해요.
...
- 이미 있는 기능은 package 설치를 우선 검토해요.
- 프로젝트 고유 절차는 `.pi/extensions`에서 시작해요.
- 여러 프로젝트에서 반복되면 package로 묶어요.
- 외부 package는 소스와 동작 범위를 확인한 뒤 설치해요.

## 10. 세션 이름 지정

작업 목적에 맞게 세션 이름을 붙여요.

```text
/name 프로젝트 초기 분석
```

이름을 붙이면 `/resume`에서 찾기 쉬워요.

## 11. 모델과 thinking level 정하기
...
프로젝트 시작 준비는 아래 상태에서 완료해요.

- 프로젝트 루트와 작업 대상이 확정됐어요.
- 변경 전 Git 상태와 검증 결과를 기록했어요.
- Project Trust 결정을 소스 검토 결과에 맞게 저장했어요.
- `AGENTS.md`에 실제 실행 명령과 프로젝트 규칙이 있어요.
