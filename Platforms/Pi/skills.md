...
| 작업 | 결과 | 예제 |
| --- | --- | --- |
| `pd` | 목표, 사용자, 요구사항, 흐름과 성공 기준 | `/skill:ct-plan pd 주문 취소 기능` |
| `review` | 근거, 발견 사항, 대안과 결정 항목 | `/skill:ct-plan review .docs/payment-design.md` |
| `impl` | 변경 대상, 영향, 구현 순서와 검증 기준 | `/skill:ct-plan impl 쿠폰 중복 적용 방지` |
| `improve` | 현황, 개선 과제, 우선순위와 기대 효과 | `/skill:ct-plan improve 회원 모듈` |
...
```text
/skill:ct-spring 주문 취소 트랜잭션을 구현해줘
/skill:ct-spring Security 설정을 검토해줘
/skill:ct-spring component API 예외 응답 처리를 추가해줘
```
...
```text
/skill:ct-script-run macOS 개발 실행 스크립트를 만들어줘
/skill:ct-script-run Windows에서 API와 프론트를 함께 실행하게 해줘
```
...
| 작업 | 결과 | 예제 |
| --- | --- | --- |
| `analyze` | 호출과 데이터 흐름 | `/skill:ct-calltree analyze OrderController.cancel` |
| `transition` | 현재·목표 구조와 전환 계획 | `/skill:ct-calltree transition OrderService.cancel` |
| `test` | 호출 흐름 기반 테스트와 검증 | `/skill:ct-calltree test .docs/order-cancel-calltree.md` |
...
```text
/skill:ct-qa-lucin 주문 취소 기능을 오픈 전 검증해줘
/skill:ct-qa-lucin 로그인 API와 화면 흐름의 회귀 범위를 정리해줘
```
...
데이터베이스 환경, 스키마와 실행 근거를 수집해 SQL 성능 원인을 분석하고 개선안을 검증해요.

```text
/skill:ct-query-tuner 주문 조회 쿼리가 느린 원인을 분석해줘
/skill:ct-query-tuner src/main/resources/mapper/OrderMapper.xml의 findOrders
```
...
```text
/skill:ct-external-architect 기존 PG 연동을 새 공급자로 이관하는 계획을 작성해줘
/skill:ct-external-architect 결제 승인·취소 연동 구조를 검토해줘
```
...
```text
/skill:ct-wiki-api 333 페이지를 조회해줘
/skill:ct-wiki-api 결제 개편 문서를 검색해줘
/skill:ct-wiki-api 333 페이지 수정안을 dry-run으로 확인해줘
```

조회·검색·저장과 변경 작업은 `/skill:ct-wiki-api <도구 작업> ?`에서 필요한 인자를 확인할 수 있어요.
...
```text
/skill:ct-wiki-ops search 결제 개편 결정 사항은?
/skill:ct-wiki-ops verify .wiki/payment.md
/skill:ct-wiki-ops lint
```
