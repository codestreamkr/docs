...
- MCP: 도구와 API를 Codex에 연결해요.
- App·Connector: 인증된 개인 또는 조직 서비스를 연결해요.
...
기계적으로 판정할 수 있는 실행 제어에 사용해요.

- Hook: 도구 실행과 작업 생명주기 이벤트에 연결해요.
- Rules: 실행 명령의 허용, 확인과 차단을 판정해요.

프로젝트의 의미와 맥락을 판단해야 하는 기준은 `AGENTS.md`나 필요한 Skill에서 제공해요.

## Plugin

여러 사람이 설치할 수 있도록 Skill과 연결 구성을 하나의 단위로 배포할 때 사용해요.

하나의 개인 작업 절차만 필요하면 Skill로 시작하고, 여러 Skill·MCP·자산을 함께 배포할 때 Plugin을 검토해요.
